...
For this we will create a new file called `AssetNode.cs` located in the `Types` directory.
...
The class `AssetNode` is annotated with the `ExtendObjectTypeAttribute` which allows us to extend an existing type with addition fields.
...
Since we have annotated the class with the `ExtendObjectTypeAttribute` it will be automatically included into the assembly GraphQL module which is generated by the **HotChocolate** source generator.
...
Last the all **DataLoader** will be automatically added.

- IDataLoader

:::

The GraphQL module name is defined in the `ModuleInfo.cs`.

```csharp title="/ModuleInfo.cs"
[assembly: Module("AssetTypes")]
```

In order to register this module with the GraphQL configuration head over to the `Program.cs` and chain in `AddAssetTypes` to the GraphQL configuration.
