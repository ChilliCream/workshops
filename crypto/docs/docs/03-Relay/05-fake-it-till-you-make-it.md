...
Another change is to enhance the `fetchFn` and `subscribeFn` to handle multipart responses, file upload and subscriptions. A more extensive solution will be located into the file `/client/index.js`. Even so, it is possible and recommended to make it extensible with a middleware system to enable other features.
