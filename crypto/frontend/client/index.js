...
class SusbcriptionProvider {
  wsClient = null;
  manager = null;

  constructor(library) {
    switch (library) {
      case 'subscriptions-transport-ws':
        this.manager = this.withSTWS;
        break;
      case 'graphql-ws':
        this.manager = this.withGWS;
        break;
      default:
        throw new Error('Wrong subscriptions library');
    }
  ...
  /** With `graphql-ws`. */
  withGWS = (operation, variables) => {
    const wsEndpoint = Config.WS_ENDPOINT;
    const authToken = Config.AUTH_TOKEN;

    const client = (this.wsClient ??= createClient({
      url: wsEndpoint,
      connectionParams: {
        Authorization: authToken ? `basic ${authToken}` : undefined,
      },
  ...
  withSTWS = (operation, variables) => {
    const wsEndpoint = Config.WS_ENDPOINT;
    const authToken = Config.AUTH_TOKEN;

    const client = (this.wsClient ??= new SubscriptionClient(wsEndpoint, {
      reconnect: true,
      connectionParams: {
        Authorization: authToken ? `basic ${authToken}` : undefined,
      },
...
const subscribeFn = new SusbcriptionProvider('graphql-ws');
// const subscribeFn = new SusbcriptionProvider('subscriptions-transport-ws');
